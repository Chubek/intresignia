...
def intresignia_detect_alt(img_path: str, stn: st.Settings, pyrd=True) -> np.array:
    ...
    for i, circle in enumerate(circles):
        ...
        p("Checking whether the image has the red color...")

        whether_red = color.detect_red(
            img_cropped, stn.color_low, stn.color_high)

        if not whether_red:
            p("No red detected, continuing...")
            continue

        p("Detecting circles again...")
        ...
        if not circles_:
            p("No circles found inside the cropped image, contunuing...")
            continue
    ...
    return output, color_isolated,img_preprocessed, res
